...
/**
 * Initiates a Direct-to-Drive upload session for an asset or reference.
 * Bypasses Vercel 4.5 MB body size limit.
 */
export async function getAssetUploadSession({
  projectId,
  fileName,
  mimeType,
  fileSize,
  ...
  try {
    const { uploadUrl, subfolderId } = await createResumableUploadSession(
      user.id,
      project.drive_folder_id,
      subfolder,
      fileName,
    ...
    return { success: true, uploadUrl, subfolderId }
  } catch (err: unknown) {
    console.error('Failed to initialize asset upload session:', err)
    return {
      success: false,
...
/**
 * Records the uploaded asset in Supabase after direct-to-drive completion.
 */
export async function recordAssetAfterUpload({
  projectId,
  name,
  type,
  taskId,
  ...
  const { data: assetData, error: insertError } = await supabase
    ...
    .insert({
      ...
      status: driveFileId && status === 'todo' ? 'done' : status,
      needs_credit: needsCredit,
  ...
  if (insertError || !assetData) {
    console.error('Error inserting asset after upload:', insertError)
    return { success: false, error: insertError?.message || 'Failed to save asset record' }
  }

  // 2. Insert reference files (if any uploaded directly)
  if (referenceFiles && referenceFiles.length > 0 && user) {
  ...
  // 3. Insert credit attribution (if requested)
  if (needsCredit && creditInfo && creditInfo.sourceName?.trim()) {
    await supabase.from('credits').insert({
      project_id: projectId,
      asset_id: assetData.id,
      source_name: creditInfo.sourceName.trim(),
      author: creditInfo.author?.trim() || null,
      license: creditInfo.license || 'cc0',
      source_url: creditInfo.sourceUrl?.trim() || null,
      notes: creditInfo.notes?.trim() || null,
    })
    revalidatePath(`/projects/${projectId}/credits`)
  }
...
/**
 * Records single asset file upload after direct-to-drive completion.
 */
export async function recordSingleAssetFileUpload({
  assetId,
  projectId,
  driveFileId,
  fileName,
...
/**
 * Records asset bundle after direct-to-drive completion.
 */
export async function recordAssetBundleAfterUpload({
  projectId,
  name,
  driveFileId,
  fileName,
